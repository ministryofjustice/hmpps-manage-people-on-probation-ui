...
const fillFilters = (page: ActivityLogPage) => {
  ...
  page.getContactTypeFilter(1).click()
  page.getContactStatusFilter(1).click()
  page.getContactStatusFilter(2).click()
  page.getContactStatusFilter(3).click()
}
...
const filtersAreFilled = (page: ActivityLogPage) => {
  ...
  page.getContactTypeFilter(1).should('be.checked')
  page.getContactStatusFilter(1).should('be.checked')
  page.getContactStatusFilter(2).should('be.checked')
  page.getContactStatusFilter(3).should('be.checked')
}
...
context('Activity log', () => {
  ...
  it('should display the filter tag and filter the list if a keyword value is submitted', () => {
    ...
    page.getSelectedFilterTagsList().find('li').should('have.length', 1)
    page.getSelectedFilterTag(0, 0).should('contain.text', value)
  ...
  it('should remove the tag, clear the keyword field and reset the list if the keyword tag is clicked', () => {
    ...
    page.getKeywordsInput().type(value)
    page.getApplyFiltersButton().click()
    page.getSelectedFilterTag(0, 0).click()
    page.getSelectedFiltersBox().should('not.exist')
  ...
  it(`should display the filter tag and filter the list if a 'date from' and 'date to' are submitted`, () => {
    ...
    page.getSelectedFilterTagsList().should('have.length', 1)
    page.getSelectedFilterTag(0, 0).should('contain.text', `${fromDate} - ${toDate}`)
    page.getCardHeader('timeline1').should('contain.text', 'Phone call from Eula Schmeler')
  ...
  it('should display the 3 filter tags and filter the list if all contact type and status filters as clicked and submitted', () => {
    cy.visit('/case/X000001/activity-log')
    const page = Page.verifyOnPage(ActivityLogPage)
    page.getContactTypeFilter(1).click()
    page.getContactStatusFilter(1).click()
    ...
    page.getContactStatusFilter(3).click()
    page.getApplyFiltersButton().click()
    page.getSelectedFiltersBox().find('h3:nth-of-type(1)').should('contain.text', 'Contact type')
    page.getSelectedFiltersBox().find('h3:nth-of-type(2)').should('contain.text', 'Contact status')
    page.getSelectedFiltersBox().should('exist')
    page.getContactTypeFilter(1).should('be.checked')
    page.getContactStatusFilter(1).should('be.checked')
    page.getContactStatusFilter(2).should('be.checked')
    page.getContactStatusFilter(3).should('be.checked')
    page.getSelectedFilterTagsList(0).find('li').should('have.length', 1)
    page.getSelectedFilterTagsList(1).find('li').should('have.length', 3)
    page.getSelectedFilterTag(0, 0).should('contain.text', contactFilterOptions.contactType[0].text)
    page.getSelectedFilterTag(1, 0).should('contain.text', contactFilterOptions.contactStatus[0].text)
    page.getSelectedFilterTag(1, 1).should('contain.text', contactFilterOptions.contactStatus[1].text)
    page.getSelectedFilterTag(1, 2).should('contain.text', contactFilterOptions.contactStatus[2].text)
  ...
  it(`should display 2 filter tags, uncheck the filter option and filter the list when '${contactFilterOptions.contactStatus[2].text}' tag is clicked`, () => {
    cy.visit('/case/X000001/activity-log')
    const page = Page.verifyOnPage(ActivityLogPage)
    page.getContactTypeFilter(1).click()
    page.getContactStatusFilter(1).click()
    ...
    page.getContactStatusFilter(3).click()
    page.getApplyFiltersButton().click()
    page.getSelectedFilterTag(1, 2).click()
    page.getSelectedFilterTagsList(0).find('li').should('have.length', 1)
    page.getSelectedFilterTagsList(1).find('li').should('have.length', 2)
    page.getSelectedFilterTag(0, 0).should('contain.text', contactFilterOptions.contactType[0].text)
    page.getSelectedFilterTag(1, 0).should('contain.text', contactFilterOptions.contactStatus[0].text)
    page.getSelectedFilterTag(1, 1).should('contain.text', contactFilterOptions.contactStatus[1].text)
    page.getContactTypeFilter(1).should('be.checked')
    page.getContactStatusFilter(1).should('be.checked')
    page.getContactStatusFilter(2).should('be.checked')
    page.getContactStatusFilter(3).should('not.be.checked')
    page.getCardHeader('timeline1').should('contain.text', 'AP PA - Attitudes, thinking & behaviours at 9:15am')
    page.getCardHeader('timeline2').should('contain.text', 'Pre-Intervention Session 1 at 9:15am')
    cy.get('[data-qa="results-count-start"]').should('contain.text', '1')
  ...
  it(`should display 1 filter tag, uncheck the deselected filter options and filter the list when '${contactFilterOptions.contactStatus[1].text}' and '${contactFilterOptions.contactStatus[2].text}' tags are clicked`, () => {
    cy.visit('/case/X000001/activity-log')
    const page = Page.verifyOnPage(ActivityLogPage)
    page.getContactTypeFilter(1).click()
    page.getContactStatusFilter(1).click()
    ...
    page.getContactStatusFilter(3).click()
    page.getApplyFiltersButton().click()
    page.getSelectedFilterTag(1, 1).click()
    page.getSelectedFilterTag(1, 1).click()
    page.getSelectedFilterTag(0, 0).click()
    page.getSelectedFilterTagsList(0).find('li').should('have.length', 1)
    page.getSelectedFilterTag(0, 0).should('contain.text', contactFilterOptions.contactStatus[0].text)
    page.getContactTypeFilter(1).should('not.be.checked')
    page.getContactStatusFilter(1).should('be.checked')
    page.getContactStatusFilter(2).should('not.be.checked')
    page.getContactStatusFilter(3).should('not.be.checked')
  })
  it(`should clear all selected filters when 'Clear filters' link is clicked`, () => {
    ...
    page.getSelectedFiltersBox().find('h3:nth-of-type(3)').should('contain.text', 'Contact type')
    page.getSelectedFiltersBox().find('h3:nth-of-type(4)').should('contain.text', 'Contact status')
    page.getSelectedFilterTagsList(0).find('li').should('have.length', 1)
    page.getSelectedFilterTagsList(1).find('li').should('have.length', 1)
    page.getSelectedFilterTagsList(2).find('li').should('have.length', 1)
    page.getSelectedFilterTagsList(3).find('li').should('have.length', 3)
    page.getCardHeader('timeline1').should('contain.text', 'Phone call from Eula Schmeler')
    cy.get('[data-qa="results-count-start"]').should('contain.text', '1')
    cy.get('[data-qa="results-count-end"]').should('contain.text', '1')
    cy.get('[data-qa="results-count-total"]').should('contain.text', '1')
    ...
    page.getDateFromInput().should('have.value', '')
    page.getDateToInput().should('have.value', '')
    page.getContactStatusFilter(1).should('not.be.checked')
    page.getContactStatusFilter(2).should('not.be.checked')
    page.getContactStatusFilter(3).should('not.be.checked')
    page.getCardHeader('timeline1').should('contain.text', 'Video call')
    cy.get('[data-qa="results-count-start"]').should('contain.text', '1')
