...
context('Contacts', () => {
  ...
  it('should display the filter tag and filter the list if a keyword value is submitted', () => {
    ...
    page.getActivityTitle(0).should('contain.text', 'Phone call')
  ...
  it('should display the filter tag and filter the list if a date from and date to are submitted', () => {
    ...
    cy.get('.contact-activity').should('have.length', 1)
  ...
  it('should display the 3 filter tags and filter the list if all compliance filters as clicked and submitted', () => {
    ...
    page.getActivityTitle(0).should('contain.text', 'AP PA - attitudes, thinking & behaviours')
    page.getActivityTitle(1).should('contain.text', 'Pre-intervention session 1')
    page.getActivityTitle(2).should('contain.text', 'Initial appointment - home visit (NS)')
    cy.get('[data-qa="results-count-start"]').should('contain.text', '1')
  ...
  it('Contacts page renders activities with View and Manage links', () => {
    ...
    page.getActivityViewNoNotes(0).should('contain.text', 'Planned video contact (NS)')
    page.getActivityTitle(1).should('contain.text', 'Phone call')
    page.getActivityViewNoNotes(2).should('contain.text', 'Planned appointment')
    page.getActivityViewNoNotes(3).should('contain.text', 'Initial appointment')
    page.getActivityViewNoNotes(4).should('contain.text', 'Office appointment')
