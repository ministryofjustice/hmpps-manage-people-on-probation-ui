...
import { filterContacts, filterContactsMonths } from './filterContacts'
...
describe('filterContactsMonths', () => {
  ...
  it('filters contacts within last 3 months (default)', () => {
    const result = filterContactsMonths(outcomes)
  ...
  it('respects custom month', () => {
    const result = filterContactsMonths(outcomes, 1)
  ...
  it('returns empty array if no matches', () => {
    const result = filterContactsMonths(outcomes, 0)

    expect(result).toEqual([])
  })

  it('handles empty input', () => {
    const result = filterContactsMonths([])

    expect(result).toEqual([])
  })

  it('handles undefined input safely', () => {
    const result = filterContactsMonths(undefined as any)
