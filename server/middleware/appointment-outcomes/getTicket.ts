    ...
    if (['attended-failed-to-comply', 'unacceptable-absence'].some(url => reqUrl.endsWith(url))) {
      ...
      if (failureToComplyContacts?.length > 1 && compliance?.[priorBreachOrRecallCountKey] === 0) {
        ticket = {
          ...
          html: `<p class="govuk-body">You should consider initiating a ${breachOrRecall}. <a class="govuk-link" href="/case/${crn}/activitylog/redirect?keywords=&compliance=not+complied&submit=true&view=&page=0" target="_blank" rel="noopener noreferrer">View a list of ${forename}’s non-compliance (opens in new tab)</a>.</p>`,
          type: 'RED',
        }
      }
