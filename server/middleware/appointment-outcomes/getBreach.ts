...
export const getBreach = async (
  ...
  const sentence = popSentences.sentences.find(({ id }) => id === selectedSentence)
  if (!sentence) return null

  return (
    compliance.currentSentences.find(
      currentSentence => currentSentence.activeBreach && currentSentence.eventNumber === sentence.eventNumber,
    ) ?? null
  )
}
