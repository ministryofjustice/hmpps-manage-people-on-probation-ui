...
describe('getTierDetails middleware', () => {
  ...
  it('should set tierDetails to undefined when the API returns a non-200 status', async () => {
    ...
    expect(req.session.data.personalDetails[CRN].tierDetails).toBeUndefined()
    expect(nextSpy).toHaveBeenCalled()
  })
  ...
  it('should lowercase tierScore and set provisional to false when tierScore is MISSING and provisional is true', async () => {
  ...
  it('should only lowercase tierScore when tierScore is MISSING and provisional is already false', async () => {
    mpopComponents.getTierDetails.mockResolvedValue({
      calculation: {
        tierScore: 'MISSING',
        provisional: false,
  ...
  it('should not modify tierScore when it is not MISSING', async () => {
    const tierResponse = makeTierResponse()
    mpopComponents.getTierDetails.mockResolvedValue(tierResponse)
    const req = buildReq({ [CRN]: makeSessionEntry() })
    const res = buildRes(true)
