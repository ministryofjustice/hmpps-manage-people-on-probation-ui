...
function getRiskBadgeText(description: string, level: string): string {
  const text = toSentenceCase(description, [], null, true, false)

  const isPublicProtection = registerTypePriority.some(
    registerType =>
      registerTypes[registerType].filter(
        ({ description: registerDescription }) =>
          registerType === 'publicProtection' &&
          registerDescription.trim().toLowerCase() === description.trim().toLowerCase(),
      ).length > 0,
  )

  if (isPublicProtection) {
