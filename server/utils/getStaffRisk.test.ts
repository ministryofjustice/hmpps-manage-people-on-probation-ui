...
import { getProbationRisk, getStaffRisk } from './getStaffRisk'
...
describe('getProbationRisk()', () => {
  ...
  it('should return the single "Risk to Probation Staff" flag if it exists', () => {
    ...
    expect(getProbationRisk(mockRiskFlags)).toEqual(
      mockRiskFlags.filter(riskFlag => riskFlag.description === 'Risk to Probation Staff')[0],
    )
  })
})
