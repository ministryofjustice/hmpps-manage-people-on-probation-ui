...
export const deliusDeepLinkUrl = (component: string, crn: string, contactId?: string) => {
  if (!component || !crn) {
    return ''
  }

  return `${config.delius.link}/NDelius-war/delius/JSP/deeplink.xhtml?component=${component}&crn=${crn}${contactId ? `&componentId=${contactId}` : ''}`
}
